...
const getFeedbackMessage = function (guess, secretNumber) {
  if (guess === secretNumber) {
    return 'Correct Number!';
  } else if (Math.abs(guess - secretNumber) <= 3) {
    // Close to the secret number
    return guess > secretNumber ? 'High, but very close!' : 'Low, but very close!';
  } else if (guess > secretNumber) {
    return guess - secretNumber <= 5 ? 'High!' : 'Too high!';
  } else {
    return secretNumber - guess <= 5 ? 'Low!' : 'Too low!';
  }
};
...
document.querySelector('.check').addEventListener('click', function () {
  ...
  if (!guess) {
    ...
    if (score > 1) {
      displayMessage(getFeedbackMessage(guess, secretNumber), defaultMessageClass);
      score--;
